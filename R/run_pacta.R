...
run_pacta <- function(config) {
  ...
  for (vals in sector_vals) {
    ...
    results_obj <- paste0("results_", vals[["method"]], "_total")
    groups <- unique(get(results_obj)[[by_group]])
    metric_col <- ifelse(vals[["method"]] == "tms", "metric", "emission_factor_metric")
    
    for (i in groups) {
      available_rows <- get(results_obj) %>%
        dplyr::filter(
          .data[[by_group]] == .env[["i"]],
      ...
      if (available_rows > 0) {
        generate_individual_outputs(
          data = get(results_obj),
          matched_prioritized = matched_prioritized,
          output_directory = output_path_standard,
          target_type = vals[["method"]],
          by_group = by_group,
          by_group_value = i,
